...
// Returns [{ text, quoted, key, val }] where:
...
function tokenize(input) {
  ...
  while (i < n) {
    ...
    while (i < n && !isSpace(input[i])) {
      ...
      if (ch === ":" && inPrefix && colonAt === -1) colonAt = text.length;
      text += ch;
      i++;
    }
    let key = null;
    ...
    tokens.push({ text, quoted: startedWithQuote, key, val });
  }
  return tokens;
}
...
export function parseQuery(input, now = Date.now()) {
  ...
  for (let i = 0; i < tokens.length; i++) {
    const { text, quoted, key, val } = tokens[i];
    if (!text) continue;
